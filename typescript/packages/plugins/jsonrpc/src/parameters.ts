...
export class JSONRpcBodySchema extends createToolParameters( 
    z.object({
        ...
        params: z.array(z.string()).describe("A structured value that holds the parameter value to be used during the invokation of the method"),
        id: z.number().describe("An identifier established by the client that must contain a string number or null"),
        jsonrpc: z.string().describe("A string that specifies the version of the JSON-RPC protocol must be exactly {{'2.0'}}"),             
    })
){}
